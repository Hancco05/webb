...
<div class="row">
    <div class="col-md-3">
        <div class="card"><div class="card-header">Cursos</div><ul class="list-group list-group-flush"><?php foreach($cursos as $c): ?><li class="list-group-item"><a href="?curso_id=<?= $c['id'] ?>&fecha=<?= $fecha ?>"><?= htmlspecialchars($c['nombre']) ?></a></li><?php endforeach; ?></ul></div>
    </div>
    <div class="col-md-9">
        <?php if ($curso_id): ?>
        <div class="card"><div class="card-header">Asistencia - Curso <?= htmlspecialchars($cursos[array_search($curso_id, array_column($cursos, 'id'))]['nombre'] ?? '') ?> - Fecha: <input type="date" id="fecha" value="<?= $fecha ?>" onchange="cambiarFecha()"></div>
        <div class="card-body">
            ...
            <form method="POST">
                <table class="table">
                    ...
                    <tr>
                        ...
                        <td><select name="asistencia[<?= $e['id'] ?>]" class="form-select">
