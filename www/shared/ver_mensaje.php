...
if (!$mensaje || ($mensaje['to_user_id'] != $user_id && $mensaje['from_user_id'] != $user_id)) {
    die("No tienes permiso para ver este mensaje.");
}

// Marcar como leído si es destinatario
if ($mensaje['to_user_id'] == $user_id && !$mensaje['is_read']) {
    marcarMensajeComoLeido($id, $user_id);
}
...
<div class="card">
    <div class="card-header">
        <strong><?= htmlspecialchars($mensaje['asunto']) ?></strong>
        <span class="float-end"><?= $mensaje['created_at'] ?></span>
    </div>
    <div class="card-body">
    ...
    <div class="card-footer">
        <a href="enviar_mensaje.php?responder=<?= $id ?>" class="btn btn-primary">Responder</a>
        <a href="mensajes.php" class="btn btn-secondary">Volver</a>
    </div>
</div>
        ...
        <div class="card-body">
            <?php foreach($respuestas as $r): ?>
                <div class="border-bottom mb-2 pb-2">
                    <strong><?= htmlspecialchars($r['remitente_nombre']) ?></strong> - <?= $r['created_at'] ?>
